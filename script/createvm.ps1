...
function CreateResourceGroup($resourceGroupName, $vmLocation)
{
    ...
    if(!$res)
    {
        ...
        [console]::WriteLine("Resource Group: {0} already existed" -f $resourceGroupName)
    }
}
...
function CreateStorageAccount($resourceGroupName, $destStorageAccount, $vmLocation)
{
    ...
    if(!$res)
    {
        ...
        [console]::WriteLine("Storage Account: {0} already existed" -f $destStorageAccount)
    }     
...
function CreateContainer($destContainerName, $destStorageAccount,$accountKey, $endpoint, $destContext)
{
    ...
    if(!$getdestContainerName)
    {
        ...
        [console]::WriteLine("Containner: {0} already existed" -f $destContainerName)
    }
...
function CopyVHD($fileName, [Microsoft.Azure.Commands.Common.Authentication.Abstractions.IStorageContext]$destContext, $destContainerName, $fileAbsoluteUri)
{
    ...
    if(!$copyStatus)
    {
	    ...
	    [console]::WriteLine("VHD file:{0} existed, do not do copy action" -f $fileName)
	} 
...
function CreateVM($resourceGroupName, $vmLocation, $vmSize, $osType, $vmPorts, $vmUser, $vmPwd, $osDiskUrl, $vhdName, $randSuffix)
{
    ...
    $networkinterfacename = "networkinterface{0}" -f $randSuffix
    $nic = New-AzureRmNetworkInterface -Name $networkinterfacename -ResourceGroupName $resourceGroupName -Location $vmLocation -SubnetId $vnet.Subnets[0].Id -PublicIpAddressId $publicIP.Id -NetworkSecurityGroupId $nsg.Id -Force
...
function processParam($resourceGroupName)
{
    if([String]::IsNullOrEmpty($resourceGroupName) -or [String]::IsNullOrEmpty($vhduri)){
        Write-Host $helpDocument
        exit
    }
	# Process those parameters who have default values
...
function login(){
    ...
    if([String]::IsNullOrEmpty($accountStr) -or [String]::IsNullOrEmpty($accountStr.Account)) #decide if has been onboarded
    {
        ...
        Write-Host ("Logined account：{0}" -f $accountStr.Account)
        Write-Host ("Logined subscription：{0}" -f $accountStr.Name)

        if(InputValue "Logined and you need change the account?(y/n)"){
            $res = Login-AzureRmAccount -EnvironmentName Azurechinacloud
            Write-Host $res
            if(!$res -or [String]::IsNullOrEmpty($res))
            {
...
function main(){
    ...
    processParam $resourcegroup

    login
	
    $vmSizes | ForEach-Object{  

        [console]::WriteLine("`r`nBegin to create resources ... To study parameter, please execute: .\createvm.ps1 --help")
        ...
        [console]::WriteLine("`r`n1. Creating resource group:{0}" -f $resourcegroup)
        CreateResourceGroup $resourcegroup $vmLocation

        [console]::WriteLine("`r`n2. Creating storage account:{0}" -f $destStorageAccount)
        ...
        [console]::WriteLine("`r`n3. Creating storage context with account key:{0}" -f $accountKey[0].Value)
        ...
        [console]::WriteLine("`r`n4. Creating storage container:{0}" -f $destContainerName)
        CreateContainer $destContainerName $destStorageAccount $accountKey $endpoint $destContext

        [console]::WriteLine("`r`n5. Copying source VHD file to destination file:{0}. Note that the Storage Account of VHD file must be the same as the VM which is deployed" -f $destFileAbsoluteUri)
        CopyVHD $vhdName $destContext $destContainerName $vhduri

        [console]::WriteLine("`r`n6. Creating VM:")
        $vmName = CreateVM $resourcegroup $vmLocation $_ $osType $vmPorts $userName $passWord $osDiskUrl $vhdName $randSuffix
	    if($vmName)
        {
            $ip = (Get-AzureRmPublicIpAddress -Name $vmName -ResourceGroupName $resourcegroup).IpAddress
